return {
	...
	config = function()
		...
		-- BEGIN: Go attach config with prompt for Delve port
		dap.configurations.go = dap.configurations.go or {}
		table.insert(dap.configurations.go, {
			type = "go",
			name = "Attach to running Go server (prompt port)",
			request = "attach",
			mode = "remote",
			host = "127.0.0.1",
			port = function()
				local port = vim.fn.input("Delve port: ", "38697")
				return tonumber(port)
			end,
		})
		-- END: Go attach config with prompt for Delve port
		...
		-- BEGIN: previous continue keymap (kept for reference)
		-- vim.keymap.set("n", "<leader>0", dap.continue, { desc = "Debug: Start/Continue" })
		-- END: previous continue keymap
		...
		-- BEGIN: previous dapui.setup (kept for reference)
		-- dapui.setup({
		...
		-- 	},
		-- })
		-- END: previous dapui.setup

		-- BEGIN: previous minimal dapui layout (kept for reference)
		-- dapui.setup({
		-- 	icons = { expanded = "▾", collapsed = "▸", current_frame = "*" },
		-- 	controls = {
		-- 		enabled = true,
		...
		-- 	},
		-- })
		-- END: previous minimal dapui layout

		-- BEGIN: requested dapui layout (right=repl, bottom=scopes)
		dapui.setup({
			icons = { expanded = "▾", collapsed = "▸", current_frame = "*" },
			controls = {
				enabled = true,
