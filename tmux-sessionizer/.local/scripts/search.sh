...
# First show app list with drun
APP=$(rofi -show drun -show-icons -p "Launch or Search:")
...
# If gtk-launch failed (invalid desktop entry), fallback to search
if [ $? -ne 0 ]; then
    $BROWSER "${SEARCH_ENGINE}${APP// /+}" &
fi
